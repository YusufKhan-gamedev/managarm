...
async::detached Rtl8139Nic::bindDevice() {
	...
	auto filter = mbus::Conjunction({
		mbus::EqualsFilter("pci-class", "02"),  /* pci-class for networking */
 		mbus::EqualsFilter("pci-vendor", "10ec"), /* vendorID for rtl8139 */
		mbus::EqualsFilter("pci-device", "8139") /* deviceID for rtl8139 */
	});
...
async::result<void> Rtl8139Nic::receive(arch::dma_buffer_view frame) {
	...
	std::byte * receiveOut = nullptr;
	size_t bufferSize = sizeof(receiveBuffer);
	memcpy(receiveOut, receiveBuffer, bufferSize);
	...
	*/
	readPointer = (readPointer + sizeof(receiveBuffer) + 4 + 3) & (~0x3);

	/* Place the read pointer in the card */
	arch::_detail::io_ops<uint16_t>::store(0x38, port - 0x10);

	/* convert receiveOut into the frame, generic to many(if not all drivers */
	frame = arch::dma_buffer_view{nullptr, receiveOut, bufferSize};
	co_return;
}
